...
- 6개 계류장/마리나 데이터:
  ...
  - 강정마리나
  - 아라마리나
- 지역, 계류 타입, 빈자리, 총 계류 수, 월 예상 계류비, 최대 선체 길이, 수심, 연락처, 시설 태그, 서비스, 설명, 이미지 포함

**2. 계류장 찾기 화면 추가 (`src/views/Marinas.jsx`)**
- hero 영역: 전체 상담 가능 자리 수 표시
- 필터:
  - 지역
  - 필요 시설
  - 상담 가능 자리만 보기
- 카드 정보:
  ...
  - 최대 선체/수심/총 계류 수
  - 시설 태그/서비스
  - 상담 요청/전화 문의 mock CTA
- 하단 유의사항: 실제 계약 전 선체 길이, 흘수, 전기 사용량, 장기 계류 조건 확인 안내

**3. 라우팅/진입점 연결**
- `App.jsx`: `tab === 'marinas'` 라우팅 추가
- `Home.jsx`: 서비스 스트립에 `계류장` 버튼 추가
- `More.jsx`: 전체서비스에 `계류장 찾기` 버튼 추가
- `Sidebar.jsx`: PC 사이드바에 `계류장` 메뉴 추가
...
- `.marina-spec-grid`
- `.marina-services`
- PC 2열 그리드, 360px 모바일 보정

### 검증
- `source /home/hj/.nvm/nvm.sh && npm run build`: 통과
- Vite dev server + Chrome headless 390px 홈 렌더링 확인

### 남은 한계 / Claude에게 넘길 부분
- 실제 계류장 데이터가 아닌 mock 데이터
- 홈 서비스 스트립에서 `계류장`은 가로 스크롤 오른쪽에 위치함. 더 노출하고 싶으면 홈 아이콘 4개 중 하나를 계류장으로 교체하거나 별도 섹션 추가 검토
- 계류장 상세 화면/예약 폼은 미구현. 현재 상담 요청은 toast mock

### 배포 여부
로컬 수정만 완료. 필요 시 `git add/commit/push`로 Vercel 배포.
