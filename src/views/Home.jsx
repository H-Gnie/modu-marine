...
export default function Home({
  ...
  const star    = listings.filter(x => x.badges.includes('모두인증') || x.badges.includes('모두진단')).slice(0, 8)
  const videos  = listings.filter(x => x.video).slice(0, 6)
  const newest  = listings.slice(0, 6)
    ...
    <>
      ...
      <section className="hero-search">
        ...
        <div className="home-icons">
          ...
          <button onClick={() => showToast('AI시세 화면은 다음 단계에서 연결합니다')}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* ── 2-col grid on PC / stack on mobile ── */}
      <div className="pc-home-body">
        ...
        </aside>{/* end home-side-col */}

      </div>{/* end pc-home-body */}
    </>
  )
}
